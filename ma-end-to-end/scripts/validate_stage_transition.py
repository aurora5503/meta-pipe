...
def main() -> None:
    ...
    if "record_ids_match" in checks:
        ...
        if stage_pair == "02->03":
            ...
            if manifest_ids and extraction_ids:
                missing = sorted(manifest_ids - extraction_ids)
                extra = sorted(extraction_ids - manifest_ids)
                if missing or extra:
                    check_result["ok"] = False
                    if missing:
                        check_result["notes"].append(f"manifest_missing_in_extraction: {len(missing)}")
                    if extra:
                        check_result["notes"].append(f"extraction_not_in_manifest: {len(extra)}")
        else:
    ...
    if "counts_reconcile" in checks:
        ...
        if stage_pair == "02->03":
            ...
            if manifest_ids and extraction_ids and len(manifest_ids) != len(extraction_ids):
                count_result["ok"] = False
                count_result["notes"].append("manifest_records != extraction_records")
        else:
